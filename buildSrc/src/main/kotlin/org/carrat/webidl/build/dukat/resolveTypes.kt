...
private class TypeResolver(
    ...
    private fun processUnionType(unionType: IDLUnionTypeDeclaration) {
        ...
        for (member in unionType.unionMembers) {
            when (member) {
                is IDLUnionTypeDeclaration -> {
                    ...
                    newDependenciesToAdd[member.name]!!.add(
        ...
        if (unionType.originFile != null) {
            ...
            namedDeclarationsToAdd[unionType.originFile!!]!!.add(
                IDLUnionDeclaration(
                    name = unionType.name,
                    unions = listOf()
                )
            ...
            anonymousDeclarationsToAdd.add(
                IDLUnionDeclaration(
                    name = unionType.name,
                    unions = listOf()
                )
    ...
    override fun lowerTypeDeclaration(declaration: IDLTypeDeclaration, owner: IDLFileDeclaration): IDLTypeDeclaration {
        ...
        if (declaration is IDLSingleTypeDeclaration) {
            return if (!(declaration.isKnown() || userTypes.contains(declaration.name)) && !sourceSet.containsType(
                    declaration.name
                )
            ) {
...
private class DependencyResolver(val typeResolver: TypeResolver) : IDLLowering {
    ...
    override fun lowerUnionDeclaration(
        declaration: IDLUnionDeclaration,
        owner: IDLFileDeclaration
    ): IDLUnionDeclaration {
        return declaration.copy(
            unions = declaration.unions + typeResolver.getDependencies(declaration)
...
fun IDLSourceSetDeclaration.resolveTypes(userTypes: Set<String>): IDLSourceSetDeclaration {
    val typeResolver = TypeResolver(userTypes)
