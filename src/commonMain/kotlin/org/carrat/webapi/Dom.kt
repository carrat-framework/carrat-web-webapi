...
public fun Element?.childElements(name: String): List<Element> = this?.childNodes?.filterElements()?.filter { it.nodeName == name } ?: emptyList()

/** Returns the first child element of this element with the given name or null if no such element is found */
public fun Element?.firstChildElement(name: String): Element? = this?.childNodes?.filterElements()?.firstOrNull { it.nodeName == name }
