...
private class MissingMemberResolver(val context: MissingMemberContext) : IDLLowering {
    ...
    override fun lowerInterfaceDeclaration(
        ...
        declaration.parents.forEach { parent ->
            ...
            val currentParentAttributes =
                (helper.getAllInterfaceParents(resolvedParent) + resolvedParent).flatMap { it.attributes }
            val currentParentOperations =
                (helper.getAllInterfaceParents(resolvedParent) + resolvedParent).flatMap { it.operations }
        ...
        allParents.forEach { parent ->
            ...
            parent.attributes.forEach { parentAttribute ->
                if (declaration.attributes.none {
                    ...
                    conflictingAttributes.addAll(declaration.attributes.filter {
                        helper.isConflicting(
                            it,
                            parentAttribute
                        )
                    })
